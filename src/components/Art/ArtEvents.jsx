...
function ArtEvents({ onScrollToTop }) {
  const [selectedEvent, setSelectedEvent] = useState(null);

  const renderGallery = () => {
    const sortedEvents = [...EVENTS].sort((a, b) => b.id - a.id);
    const leftCol = sortedEvents.filter((_, idx) => idx % 2 === 0);
    const rightCol = sortedEvents.filter((_, idx) => idx % 2 !== 0);
    ...
    return (
      <div className="w-full px-8 lg:px-12 flex flex-col items-center">
        <div className="w-full flex flex-col md:hidden gap-2">
          {sortedEvents.map((event) => EventItem(event))}
        </div>
        <div className="hidden md:grid grid-cols-2 gap-4 w-full">
          <div className="flex flex-col gap-4">{leftCol.map((event) => EventItem(event))}</div>
          <div className="flex flex-col gap-4">{rightCol.map((event) => EventItem(event))}</div>
        </div>
  ...
  const renderModal = () => {
    ...
    const hasInfo = selectedEvent.title?.trim() || selectedEvent.description?.trim();
        ...
        >
          ...
          {hasInfo && (
            <div className="bg-dark2 relative z-20">
              {selectedEvent.title && <h2 className="text-lg md:text-xl font-bold text-white px-6 pt-4">{selectedEvent.title}</h2>}
              {selectedEvent.description && <p className="text-gray text-xs md:text-sm px-6 pb-4 pt-1">{selectedEvent.description}</p>}
            </div>
          )}
        </div>
      </div>
